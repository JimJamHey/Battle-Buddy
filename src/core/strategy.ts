...
export interface EssentialCardRef extends StrategyCardRef {
  role: string
}
...
export interface StrategyComp {
  ...
  essential: EssentialCardRef[]
  phases: StrategyPhaseView[]
...
export interface StrategyPhaseSpec {
  stage: 'early' | 'mid' | 'end'
  tiers: string
  goal: string
  cardIds?: string[]
...
export function reviewCurated(file: CuratedFile, pool: BgMinion[]): StrategyComp[] {
  ...
  const comps: StrategyComp[] = file.comps.map((row) => {
    ...
    const essential = (row.essential ?? []).map((entry) => {
      const card = byId.get(entry.id)
      return card
        ? { ...asRef(card), role: entry.role }
        : { id: entry.id, name: entry.id, techLevel: 0, role: entry.role }
    })
    const phases: StrategyPhaseView[] = (row.phases ?? []).map((phase) => ({
      stage: phase.stage,
      tiers: phase.tiers,
      goal: phase.goal,
      cards: (phase.cardIds ?? []).map((id) => {
        const card = byId.get(id)
        return card ? asRef(card) : { id, name: id, techLevel: 0 }
      })
    }))
